...
Embodied-RobotSim 是一个基于 ROS 2 (Jazzy) 构建的综合仿真工作空间。它提供了一套完整的差速驱动移动平台方案，搭载 **Franka FR3 骨干机械臂**、2D 激光雷达 (LiDAR) 以及立体 RGB-D 深度传感器。本系统在 Gazebo 仿真环境中运行，无缝集成建图、导航、先进 2D/3D 视觉感知与移动操作 (Mobile Manipulation) 等前沿的算法方案。
...
| *室内场景仿真* | *基于 YOLOE 与 GraspNet 的自主抓取* |
