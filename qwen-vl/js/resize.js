...
class ResizableLayout {
  ...
  _load() {
    try {
      const s = JSON.parse(localStorage.getItem('panel-layout-v4') || '{}');
      if (typeof s.col1     === 'number') this.state.col1     = s.col1;
      if (typeof s.col2     === 'number') this.state.col2     = s.col2;
      if (typeof s.bottomH  === 'number') this.state.bottomH  = s.bottomH;
      if (typeof s.navBasis === 'number') this.state.navBasis = s.navBasis;
    } catch(e) {}
  }

  _save() {
    localStorage.setItem('panel-layout-v4', JSON.stringify(this.state));
  }
  ...
  _initHandles() {
    ...
    this._colHandle(
      ...
      { min: 180, max: 520, def: this.defaults.col1 }
    );
    ...
    this._colHandle(
      ...
      { min: 200, max: 420, def: this.defaults.col2 }
    );
    ...
    this._rowHandle(
      ...
      { min: 180, max: 560, def: this.defaults.bottomH }
    );
