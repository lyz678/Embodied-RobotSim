...
# ─── Qwen3 Agent 后端说明 ─────────────────────────────
# Qwen3 Agent 已移至 start_llm_agent.sh 中统一启动
# 请先运行: bash start_llm_agent.sh
# Agent WebSocket: ws://localhost:${AGENT_PORT}/ws/chat
...
echo -e "     Qwen3 Agent  : ws://localhost:${AGENT_PORT}/ws/chat  (由 start_llm_agent.sh 启动)"
echo ""
echo -e "  💬 自然语言控制（右侧对话栏）："
echo -e "     例: ${CYN}帮我去厨房拿瓶可乐${NC}"
echo ""
echo -e "  🗺️  相机流示例："
